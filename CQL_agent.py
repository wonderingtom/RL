...
class PolicyNetContinuous(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        action = torch.tanh(normal_sample) # action \in [-1, 1]
...
class CQL(Agent):
    ...
    def act(self, state):
        ...
        action = self.actor(state)[0] # policy net return action and log_probs
        return action
    ...
    def update(self, transition_dict):
        ...
        random_unif_log_pi = np.log(0.01**next_actions.shape[-1]) #????????
        ...
        return min(qf1_loss.item(), qf2_loss.item()), actor_loss.item(), alpha_loss.item()
